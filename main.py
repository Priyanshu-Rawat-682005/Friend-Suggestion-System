...
def login(username, password):
    ...
    if user:
        user_id = user[0]
        print("Login successful! Your user ID is:", user_id)
        return user_id
    else:
...
def suggest_friends(user_id):
    ...
    similar = jaccard_similarity_suggestions(user_id)
    ...
    while True:
        ...
        if choice == "1":
            ...
            if not mutual and not similar:
                ...
                to_add = input("\nEnter user ID to send friend request or type 'ignore <ID>' to ignore: ").strip()
